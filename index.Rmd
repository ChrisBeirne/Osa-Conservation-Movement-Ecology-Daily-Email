...
```{r ecu1, include=FALSE}


# Get animals
ec_animals <-getMovebankAnimals(study=2885579493,login=loginStored)
ec_animals$local_identifier <- ec_animals$nick_name

# They vary by the field "sensor_type_id"
ec_animals <- ec_animals[ec_animals$sensor_type_id==653 & is.na(ec_animals$sensor_type_id)==F,]

# Get last 2 weeks
t <- now("America/Costa_Rica")
start_t <- t-as.difftime(14,units='days')

mov_dat <- getMovebankData(study=2885579493, login=loginStored,  removeDuplicatedTimestamps=TRUE,
                       timestamp_start=start_t)
...
# Convert move stack to dataframe
dat <- as.data.frame(mov_dat)
dat$local_identifier <-  dat$nick_name
table(dat$local_identifier)
# Last 24 hours
mov_dat_24 <- mov_dat[mov_dat$timestamp > t-as.difftime(1,units='days'),]
dat_24 <- as.data.frame(mov_dat_24)
table(dat_24$nick_name)

dist_24<- distance(mov_dat_24)
names(dist_24) <- mov_dat_24@idData$nick_name
#str(mov_dat_24)
dist_24<- as.data.frame((sapply(dist_24,sum)))
dist_24$local_identifier <- row.names(dist_24)
colnames(dist_24)[1] <- "dist_km_last_24h"
dist_24$dist_km_last_24h <- round(dist_24$dist_km_last_24h/1000,1)

# Convert dat to Ecuador time
dat$timestamp <- with_tz(dat$timestamp, tzone = "America/Guayaquil")

# Add the location data
dat <- left_join(dat, ec_animals[, c("tag_id", "local_identifier")])
# Sort the names out

ec_animals$timestamp_start <- ymd_hms(ec_animals$timestamp_start)
ec_animals$timestamp_end <- ymd_hms(ec_animals$timestamp_end)

# Add a country column
dat$country <- dat$location_lat<0
dat$country[dat$country==T] <- "Ecuador"
...
last_obs <- dat %>% group_by(local_identifier) %>% summarize(hours_since_fix=min(diff_time))

last_dat <- dat %>% group_by(local_identifier) %>% filter(row_number()==n())
...
# add distance move last_24
check_in_summary <- left_join(check_in_summary, dist_24, by=c("name"="local_identifier"))
