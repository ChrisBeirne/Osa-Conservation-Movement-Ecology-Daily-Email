...
warnings <- list()
counter <- 1
# Tags which have haven't checked in > 4 days, OR maximum distance moved between intervals < 100m for the last 100 fixes or total distance moved in last 100 fixes <5000m
tmp_check  <- check_in_summary[check_in_summary$time_since_checkin_h>96 & is.na(check_in_summary$dist_km_last_24h)==T,]$name

if(length(tmp_check)>0)
{
  warnings[[counter]] <- data.frame(name=tmp_check,reason="Last checkin >4 days")
  counter <- counter+1
}
...
if(length(tmp_check)>0)
{
  warnings[[counter]] <- data.frame(name=tmp_check,reason="Sum of movements low")
  counter <- counter+1
}
...
**DEATH ALERTS**
The following list shows tags which have hit one of three criteria:

They haven't reported data for > 4 days OR their maximum movement step is below 100m OR their sum of movements is very low: 
